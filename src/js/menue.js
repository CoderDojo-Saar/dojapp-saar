...
export function createSideMenu(activeMenuId) {
    ...
    if (htmlSideMenu) {
        ...
        if (isMobile()) {
            ...
            // ... zeige anstelle des Menüs einen Warnhinweis an.
            htmlSideMenu.append("Bitte schau dir diese Seite auf einem Mobilgerät an, um das Menü benutzen zu können.")
        }
    }
}


function generateSideMenuItems(htmlSideMenu, menu, activeMenuId) {
    // Fügt an das Menü jeden Eintrag an
...
function createSideMenuItem(menuItem, activeMenuId) {
    ...
    // Wenn der Eintrag leer ist, erzeuge einen Abtrenner, ansonsten erstelle
...
function createMenuItem(menuItem, activeMenuId) {
    // Wenn kein Icon für den Eintrag festgelegt ist, wird das 'link'-icon benutzt.
...
function isActiveSite(menuItem, activeMenuId) {
    var className = ' ';
...
function isMobile() {
    ...
    // Überprüft über ein langes regex, ob der Nutzer mobil ist.
